...
Mesh::Mesh(Vertex vertices[], int vertexCount, unsigned int indexArray[], int indexCount, Microsoft::WRL::ComPtr<ID3D11Device> device)
{
	CreateBuffers(vertices, vertexCount, indexArray, indexCount, device);
...
void Mesh::CreateBuffers(Vertex vertices[], int vertexCount, unsigned int indexArray[], int indexCount, Microsoft::WRL::ComPtr<ID3D11Device> device)
{
	CalculateTangents(vertices, vertexCount, indexArray, indexCount);
	...
	ibd.ByteWidth = sizeof(unsigned int) * indexCount; // Number of indices
	...
	// Save the indices
	this->m_bufferIndexCount = indexCount;
}
